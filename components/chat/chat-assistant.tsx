...
export default function ChatAssistant() {
  ...
  const handleScrapeUrl = async () => {
    ...
    try {
      if (crawlMode === 'single') {
        ...
        if (response.ok) {
          setProgressUpdate({
            ...
            status: 'completed',
            message:
              data.message || `Successfully added content from ${urlInput} to knowledge base`,
            startTime,
          });
      ...
      setTimeout(() => {
        setProgressUpdate((prev) => (prev?.status === 'completed' ? null : prev));
      }, 5000);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full">
                    ...
                    <div className="ml-12 mt-2 p-3 bg-muted/50 rounded-lg border border-border/50">
                      ...
                      <div className="space-y-2">
                        {message.sources.map((source, idx) => {
                          ...
                          const isUrl =
                            sourceStr.includes('http://') || sourceStr.includes('https://');
      ...
      <div className="p-4 border-t bg-muted/50 space-y-4">
        {progressUpdate && (
          <ProgressTracker update={progressUpdate} onClose={() => setProgressUpdate(null)} />
        )}
            ...
            {crawlMode === 'crawl' && showAdvanced && (
              <div className="flex gap-2 text-sm">
                <div className="flex items-center gap-1">
                  <label htmlFor="depth" className="text-muted-foreground">
                    Depth:
                  </label>
                  <Input
                    id="depth"
                ...
                <div className="flex items-center gap-1">
                  <label htmlFor="maxPages" className="text-muted-foreground">
                    Max Pages:
                  </label>
                  <Input
                    id="maxPages"
